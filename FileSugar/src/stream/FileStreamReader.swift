...
//         Swift.print("length:  \(length)")
         ...
         throw ("Error: \(error) reading \(url.path)")
      }
   }
}
...
extension FileStreamReader {
   ...
   static func read(filePath: String, start: UInt64, end: Int) throws -> String {
      ...
      guard let string = String(data: data, encoding: .utf8) else { throw ("FileStreamReader.read() - Unable to get string from data data.count: \(data.count)") }
      return string
   }
}
